...
The graph structure data for the metabolic networks, used as framework input (Graph-M module), are available in the following file: `Data/<model>/SMILES_node_feature_final.csv`. The graph can be built from a metabolic model (manuscript subsection "Constructing Graph Representations of Metabolic Models") by providing a COBRA-style JSON file at `Data/<model>/<model>.json` with at least `reactions` (each with `id`, `metabolites`, `gene_reaction_rule`). **JSON files are provided for the three studied models** (`e_coli_core`, `iMM904`, `iML1515`) in `Data/<model>/<model>.json`, so the pipeline uses the manuscript 4-step metabolite graph by default for these models. If the JSON is absent for a model, a random graph is used as fallback.
