...
from .create_dataset_baseline import create_gene_metabolite_dataset
from .evaluations import *
from .initialize_model_DNN import initialize_model_DNN 
###################################################
...
def main():
    ...
    # Tokenizing/padding gene sequences (do this before data filtering)
    ...
    gene_sequences_padded = pad_sequences(tokenizer.texts_to_sequences(gene_sequences), maxlen=max_len)
    
    # Tokenizing/padding meta sequences (do this before data filtering)
    ...
    smiles_features_padded = pad_sequences(smiles_tokenizer.texts_to_sequences(smiles_features), maxlen=smiles_max_len)
    ...
    for meta_name in train_metabolites:
        ...
        for gene_name, label in zip(gene_names_train, labels_train):
            gene_index = np.where(gene_names == gene_name)[0][0]  # Use numpy where for array
            meta_index = np.where(metabolite_names == meta_name)[0][0] # Get index from numpy array
            X_genes_train.append(gene_sequences_padded[gene_index])
            X_smiles_train.append(smiles_features_padded[meta_index])
    ...
    for meta_name in val_metabolites:
        ...
        for gene_name, label in zip(gene_names_val, labels_val):
            gene_index = np.where(gene_names == gene_name)[0][0]  # Use numpy where for array
            meta_index = np.where(metabolite_names == meta_name)[0][0]  # Get index from numpy array
            X_genes_val.append(gene_sequences_padded[gene_index])
            X_smiles_val.append(smiles_features_padded[meta_index])
            y_val.append(label)
    ...
    summary = calculate_metrics_for_val_metabolites(model, eval_metabolites, metabolite_names, gene_sequences_padded, smiles_features_padded, relationships, device, genes_to_exclude)
    if args.output_metrics_csv and summary is not None:
        ...
        with open(args.output_metrics_csv, 'a', newline='') as f:
            ...
            w.writerow([CBM, 'DNN', args.split_seed, round(summary[0], 4), round(summary[1], 4), round(summary[2], 4), round(summary[3], 4), round(summary[4], 4)])
