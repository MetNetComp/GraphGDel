...
def predict_gene_deletions_for_metabolite(model, metabolite_name, metabolite_names, gene_sequences, smiles_features, relationships, device, genes_to_exclude):
    ...
    with torch.no_grad():
        ...
        # Ensure predicted_probs is always an array
        if predicted_probs.ndim == 0:
        ...
        # Ensure filtered_indices are within bounds
        max_index = len(predicted_deletions) - 1
        filtered_indices = [i for i in filtered_indices if i <= max_index]
