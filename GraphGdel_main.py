...
def load_graph_data(graph_path, CBM):
    """Load graph structure data for the metabolic network."""
    try:
        # Load node features
        node_features_df = pd.read_csv(graph_path)
        
        # Create graph structure (simplified - you may need to adjust based on your data format)
        # This assumes the CSV contains node features and edge information
        num_nodes = len(node_features_df)
        
        # Create a simple graph structure (you may need to modify this based on your actual data)
        # For now, we'll create a random graph structure
        edge_index = torch.randint(0, num_nodes, (2, num_nodes * 2))
        edge_index = to_undirected(edge_index)
        
        # Create node features (assuming the CSV contains feature columns)
        node_features = torch.randn(num_nodes, 64)  # Default feature dimension
        ...
        # Return a default graph structure
        num_nodes = 1000  # Default number of nodes
...
def main():
    ...
    # Load graph data
    graph_data, num_nodes = load_graph_data(graph_path, CBM)
    ...
    # Load pretrained model (using new_code_model as requested)
    try:
        checkpoint = torch.load(f'train_disc/new_code_model_{CBM}.sav', map_location=device)
        model.load_state_dict(checkpoint, strict=False)
        print(f"Loaded pretrained model new_code_model_{CBM}.sav")
    except Exception as e:
        ...
        print("For actual results, please ensure the pretrained model architecture matches the current implementation.")
    ...
    def evaluate_per_metabolite(
        ...
        print(f"Overall Accuracy: {np.mean(acc_list):.2f}%")
        print(f"Macro-Averaged Precision: {np.mean(prec_list):.2f}%")
        print(f"Macro-Averaged Recall: {np.mean(rec_list):.2f}%")
        print(f"Macro-Averaged F1 Score: {np.mean(f1_list):.2f}%")

    # Run the exact notebook evaluation
    evaluate_per_metabolite(
        model=model,
